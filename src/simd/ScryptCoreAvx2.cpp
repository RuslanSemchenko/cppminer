// 8-way AVX2 scrypt ROMix - same technique as ScryptCoreSse2.cpp's 4-way
// version, twice as wide. Compiled with AVX2 codegen enabled for this
// translation unit only (see miner/CMakeLists.txt); every other file
// stays on the compiler's default baseline so the binary still runs on
// CPUs without AVX2 - see simd/CpuFeatures.h for the runtime check gating
// calls into here.
...
inline V rotl(V a, int n) { return _mm256_or_si256(shl(a, n), shr(a, 32 - n)); }
inline V load(const uint32_t* p) { return _mm256_loadu_si256(reinterpret_cast<const __m256i*>(p)); }
inline void store(uint32_t* p, V v) { _mm256_storeu_si256(reinterpret_cast<__m256i*>(p), v); }

// Same structure/index pattern as the scalar xorSalsa8 (ScryptCoreScalar.cpp)
// and the reference miner's xor_salsa8, just W8-wide per operation.
...
void scryptRomix8way(uint32_t B[32 * W8], uint32_t* V_, uint32_t costN)
{
    V x[32];
    for (int i = 0; i < 32; i++)
        x[i] = load(B + i * W8);
    ...
    for (uint32_t row = 0; row < costN; row++) {
        uint32_t* dst = V_ + size_t(row) * 32 * W8;
        for (int i = 0; i < 32; i++)
            store(dst + i * W8, x[i]);
        xorSalsa8(x + 0, x + 16);
        xorSalsa8(x + 16, x + 0);
    ...
    for (uint32_t row = 0; row < costN; row++) {
        V jVec = andv(x[16], _mm256_set1_epi32(static_cast<int>(costN - 1)));
        uint32_t j[W8];
        store(j, jVec);
        for (int i = 0; i < 32; i++) {
            uint32_t gathered[W8];
            for (int lane = 0; lane < W8; lane++)
                gathered[lane] = V_[size_t(j[lane]) * 32 * W8 + size_t(i) * W8 + lane];
