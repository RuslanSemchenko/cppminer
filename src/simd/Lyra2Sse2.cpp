...
inline __m128i ror64(__m128i x, int bits)
{
    return _mm_or_si128(_mm_srli_epi64(x, bits), _mm_slli_epi64(x, 64 - bits));
}

inline void g(__m128i& a, __m128i& b, __m128i& c, __m128i& d)
{
    a = _mm_add_epi64(a, b);
    d = ror64(_mm_xor_si128(d, a), 32);
    c = _mm_add_epi64(c, d);
    b = ror64(_mm_xor_si128(b, c), 24);
    a = _mm_add_epi64(a, b);
    d = ror64(_mm_xor_si128(d, a), 16);
    c = _mm_add_epi64(c, d);
    b = ror64(_mm_xor_si128(b, c), 63);
}

inline void lyraRound(std::array<__m128i, 16>& v)
{
    g(v[0], v[4], v[8], v[12]);
    g(v[1], v[5], v[9], v[13]);
    g(v[2], v[6], v[10], v[14]);
...
inline void lyraReduced(std::array<__m128i, 16>& v)
{
    lyraRound(v);
}

inline void lyraFull(std::array<__m128i, 16>& v)
{
    for (int i = 0; i < 12; ++i)
        lyraRound(v);
}

inline __m128i loadMatrix(const std::vector<uint64_t>& matrix, size_t row, size_t word)
{
    const size_t base = (row * kRowWords + word) * kLanes;
    return _mm_loadu_si128(reinterpret_cast<const __m128i*>(matrix.data() + base));
}

inline void storeMatrix(std::vector<uint64_t>& matrix, size_t row, size_t word, __m128i value)
{
    const size_t base = (row * kRowWords + word) * kLanes;
    _mm_storeu_si128(reinterpret_cast<__m128i*>(matrix.data() + base), value);
}

inline __m128i loadInput(const std::vector<uint64_t>& input, size_t block, size_t word)
{
    const size_t base = (block * kInputBlockWords + word) * kLanes;
    return _mm_loadu_si128(reinterpret_cast<const __m128i*>(input.data() + base));
}

inline void storeOutput(const std::array<__m128i, 16>& state, uint8_t out[kLanes][32])
{
    alignas(16) uint64_t lanes[kLanes];
    for (size_t word = 0; word < 4; ++word) {
        _mm_store_si128(reinterpret_cast<__m128i*>(lanes), state[word]);
        for (size_t lane = 0; lane < kLanes; ++lane)
            std::memcpy(out[lane] + word * sizeof(uint64_t), &lanes[lane], sizeof(uint64_t));
    }
}

inline void absorbSafe(std::array<__m128i, 16>& state, const std::vector<uint64_t>& input, size_t blocks)
{
    for (size_t block = 0; block < blocks; ++block) {
        for (size_t word = 0; word < kInputBlockWords; ++word)
            state[word] = _mm_xor_si128(state[word], loadInput(input, block, word));
        lyraFull(state);
    }
}

inline void squeezeRow0(std::array<__m128i, 16>& state, std::vector<uint64_t>& matrix)
{
...
inline void duplexRow1(std::array<__m128i, 16>& state, std::vector<uint64_t>& matrix)
{
    for (size_t col = 0; col < kCols; ++col) {
        for (size_t word = 0; word < kBlockWords; ++word)
            state[word] = _mm_xor_si128(state[word], loadMatrix(matrix, 0, col * kBlockWords + word));
        lyraReduced(state);
        const size_t outCol = kCols - 1 - col;
        for (size_t word = 0; word < kBlockWords; ++word) {
            __m128i in = loadMatrix(matrix, 0, col * kBlockWords + word);
            storeMatrix(matrix, 1, outCol * kBlockWords + word, _mm_xor_si128(in, state[word]));
        }
    }
}

inline void duplexSetup(std::array<__m128i, 16>& state, std::vector<uint64_t>& matrix,
                        size_t prev, size_t rowa, size_t row)
{
    for (size_t col = 0; col < kCols; ++col) {
        for (size_t word = 0; word < kBlockWords; ++word) {
            __m128i in = loadMatrix(matrix, prev, col * kBlockWords + word);
            __m128i inout = loadMatrix(matrix, rowa, col * kBlockWords + word);
            state[word] = _mm_xor_si128(state[word], _mm_add_epi64(in, inout));
        }
        lyraReduced(state);
        const size_t outCol = kCols - 1 - col;
        for (size_t word = 0; word < kBlockWords; ++word) {
            __m128i in = loadMatrix(matrix, prev, col * kBlockWords + word);
            storeMatrix(matrix, row, outCol * kBlockWords + word, _mm_xor_si128(in, state[word]));
        }
        for (size_t word = 0; word < kBlockWords; ++word) {
            __m128i inout = loadMatrix(matrix, rowa, col * kBlockWords + word);
            __m128i rotated = word == 0 ? state[11] : state[word - 1];
            storeMatrix(matrix, rowa, col * kBlockWords + word, _mm_xor_si128(inout, rotated));
        }
    }
}

inline void duplexWandering(std::array<__m128i, 16>& state, std::vector<uint64_t>& matrix,
                            size_t prev, const std::array<size_t, kLanes>& rowa, size_t row)
{
    for (size_t col = 0; col < kCols; ++col) {
        std::array<__m128i, kBlockWords> inout{};
        for (size_t word = 0; word < kBlockWords; ++word) {
            inout[word] = _mm_set_epi64x(
            ...
            __m128i in = loadMatrix(matrix, prev, col * kBlockWords + word);
            state[word] = _mm_xor_si128(state[word], _mm_add_epi64(in, inout[word]));
        }
        lyraReduced(state);
        ...
        if (!hasOverlap) {
            alignas(16) uint64_t ioLanes[kLanes];
            for (size_t word = 0; word < kBlockWords; ++word) {
                const __m128i rotated = word == 0 ? state[11] : state[word - 1];
                const __m128i updatedOut = _mm_xor_si128(
                    loadMatrix(matrix, row, col * kBlockWords + word), state[word]);
                storeMatrix(matrix, row, col * kBlockWords + word, updatedOut);

                const __m128i updatedIo = _mm_xor_si128(inout[word], rotated);
                _mm_store_si128(reinterpret_cast<__m128i*>(ioLanes), updatedIo);
                for (size_t lane = 0; lane < kLanes; ++lane)
                    matrix[(rowa[lane] * kRowWords + col * kBlockWords + word) * kLanes + lane] = ioLanes[lane];
            }
        } else {
            alignas(16) uint64_t stateLanes[kBlockWords][kLanes];
            alignas(16) uint64_t inoutLanes[kBlockWords][kLanes];
            for (size_t word = 0; word < kBlockWords; ++word) {
                _mm_store_si128(reinterpret_cast<__m128i*>(stateLanes[word]), state[word]);
                _mm_store_si128(reinterpret_cast<__m128i*>(inoutLanes[word]), inout[word]);
            }
            for (size_t word = 0; word < kBlockWords; ++word) {
                    ...
                    uint64_t firstNonce, uint32_t timeCost, uint8_t out[kLanes][32])
{
    ...
    std::array<__m128i, 16> state;
    for (size_t word = 0; word < 8; ++word)
        state[word] = _mm_setzero_si128();
    const uint64_t iv[8] = {
    ...
    for (size_t word = 0; word < 8; ++word)
        state[8 + word] = _mm_set1_epi64x(static_cast<long long>(iv[word]));
    ...
    for (uint32_t tau = 1; tau <= timeCost; ++tau) {
        ...
        do {
            alignas(16) uint64_t state0[kLanes];
            _mm_store_si128(reinterpret_cast<__m128i*>(state0), state[0]);
            for (size_t lane = 0; lane < kLanes; ++lane)
    ...
    for (size_t word = 0; word < kBlockWords; ++word) {
        ...
        state[word] = _mm_xor_si128(state[word], io);
    }
    lyraFull(state);
